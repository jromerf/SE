...
-- denounc0: debouncer port map(Bus2IP_Reset,Bus2IP_Clk,push(0),xDeb_aux0,push0_aux,xDebRisingEdge_aux0);
-- denounc1: debouncer port map(Bus2IP_Reset,Bus2IP_Clk,push(1),xDeb_aux1,push1_aux,xDebRisingEdge_aux1);
-- denounc2: debouncer port map(Bus2IP_Reset,Bus2IP_Clk,push(2),xDeb_aux2,push2_aux,xDebRisingEdge_aux2);
-- denounc3: debouncer port map(Bus2IP_Reset,Bus2IP_Clk,push(3),xDeb_aux3,push3_aux,xDebRisingEdge_aux3);

-- push_total <= push0_aux & push1_aux & push2_aux & push3_aux;
...
--Puedo determinar que los bits que no uso sean para saber que casilla mover
		...
		begin
			...
			case currentStateLectura is
				...
				when estadoEsperaLectura =>
					if (WFIFO2IP_empty = '0') then--cuando la fifo no esta vacia
						fifo_rdreq_cmb <= '1';
				...
				when estadoEnviarDato =>
					load <= '1';
					incContador <= '1';
					if (ciclo = timeOut) then--espera 4 ciclos de reloj para sincronizar con el otro
